...
jQuery(document).ready(function($) {
  ...
  $(".resume-summary").on(
    "click",
    "button.remove_educational_qualification",
    function(e) {
      e.preventDefault();
  ...
  $(".resume-summary").on("click", "button.remove_working_experience", function(
    e
  ) {
    e.preventDefault();
          ...
          try {
            ...
            if (ajaxResponse.error) {
              ...
              $.each(ajaxResponse.error_field, function(i) {
                $("label#" + ajaxResponse.error_field[i] + "-error").text(
                  ajaxResponse.message[ajaxResponse.error_field[i]]
                );
              });
            } else if (ajaxResponse.successmessage) {
              $("#cbx-formalert")
                .addClass("alert alert alert-success")
                .html(ajaxResponse.successmessage);
              $contactForm[0].reset();
            }
          } catch (e) {}
